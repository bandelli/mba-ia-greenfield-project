...
# task-next-frontend-config-base — Context
...
| next-frontend-config-base/TD-03 | ad-hoc | Cross-layer | API URL Key Strategy for the FE↔BE Bridge | decided | A (Strict BFF — single server-only `API_URL`) | — |

_Source files:_

- next-frontend-config-base — `docs/decisions/technical-decisions-next-frontend-config-base.md` (scope_type: ad-hoc, related_phases: [])
...
| Layouts (`app/**/layout.tsx`) | E2E only — when the layout adds logic (auth gate, conditional rendering); otherwise skip |
| Client components (`"use client"`) | Vitest unit (`*.test.ts`) — render with RTL; mock `next/navigation` and `fetch` |
| Feature components (`components/<feature>/*.tsx`, server, presentational) | Skip — covered via consumers / page E2E |
| shadcn UI primitives (`components/ui/*.tsx`) | None — trust the library; cover via consumers |
| Icons (`components/icons/*.tsx`) | None |
| `lib/` utilities with branching | Vitest unit (`*.test.ts`) |
| Custom hooks (`hooks/*.ts`) | Vitest unit (`*.test.ts`) — `renderHook` from `@testing-library/react` |
...
| Route handlers — simple proxy | `*.integration.test.ts` with MSW only |
