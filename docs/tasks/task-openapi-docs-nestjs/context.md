...
|     └─ Last revision: 2026-05-12 — Clarifies that the CLI plugin (`classValidatorShim: true`) covers only infe… | | | | | | |
| openapi-docs-nestjs/TD-02 | ad-hoc | Cross-layer | OpenAPI Spec Artifact Strategy | decided | C (Runtime UI + exported openapi.json) | — |
| openapi-docs-nestjs/TD-03 | ad-hoc | Backend | Production Exposure Policy for Swagger UI | decided | B (Only in dev/staging via env flag) | — |
...
**Recommendation:** aligns with the defensive posture already established in phase 02 and does not compromise legitimate consumers (the `openapi.json` committed in TD-02 fulfills the role of a "spec browsable outside the UI"). Reopening as Option A or C is trivial in the future if a public-API use case appears.
**Libraries:** —
