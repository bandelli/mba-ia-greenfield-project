...
issues:
  - id: MD-1
    status: resolved
    summary: "No Cross-layer/Repo-wide TD covers FE↔BE contract-sync strategy (Decision #29)"
  ...
  - id: OQ-8
    status: resolved
    summary: "Inventory — Account confirmation has no UI (de-scoped 2026-05-14); TD-07 needs the screen to close end-to-end flow"
  ...
  - id: OQ-10
    status: resolved
    summary: "Inventory — Reset-password destination screen missing from Figma; password recovery only partially covered"
    resolved_by: deferred_capability
  - id: OQ-11
    status: resolved
  ...
  - id: UIG-1
    status: resolved
    summary: "Capability 'Registration, login, account confirmation, and password recovery screens' covered by TD-01/TD-04 but no verb matches the umbrella bullet literally in inventory"
    resolved_by: deferred_capability
advisories:
  - id: MC-cross-1
    status: open
    summary: "Phase 02 capability 'Transactional email sending service' not claimed by any slice's covers_capabilities"
  - id: MC-cross-2
    status: open
    summary: "Phase 02 capability 'User registration with email and password' not claimed by any slice's covers_capabilities"
  - id: MC-cross-3
    status: open
    summary: "Phase 02 capability 'Automatic creation of the user's channel from the email prefix' not claimed by any slice's covers_capabilities"
  - id: MC-cross-4
    status: open
    summary: "Phase 02 capability 'Account confirmation via email with an activation link' not claimed by any slice's covers_capabilities"
  - id: MC-cross-5
    status: open
    summary: "Phase 02 capability 'Login and user session control' not claimed by any slice's covers_capabilities"
  - id: MC-cross-6
    status: open
    summary: "Phase 02 capability 'Logout' not claimed by any slice's covers_capabilities"
  - id: MC-cross-7
    status: open
    summary: "Phase 02 capability 'Password recovery: request via email → link with token → reset' not claimed by any slice's covers_capabilities"
  - id: MC-cross-8
    status: open
    summary: "Phase 02 capability 'Registration, login, account confirmation, and password recovery screens' not claimed by any slice's covers_capabilities"
---
...
_None._ All 7 current-scope TDs are `Scope: Frontend`; `## UI Inventory` is populated (3 screens), so the Scope-Subsection orphan check does not fire. Every capability cited by an inventory verb (`Registration…`, `Login…`, `Password recovery…`) is present in scope.
...
_None._ The Decision #29 FE↔BE contract-sync strategy remains covered (MD-1, resolved). The two phase-level bullets without a covering TD ("Transactional email sending service", "Automatic creation of the user's channel…") are owned by the sibling backend slice `phase-02-auth`, not by this frontend slice — surfaced as cross-slice advisories below, not as MD against this slice.
...
_None._ Capabilities without an inventory verb (`Account confirmation…`, `Logout`, the reset-password destination of `Password recovery…`, and the umbrella `Registration, login, account confirmation, and password recovery screens`) are all already recorded in `## Non-UI / Deferred Capabilities` (Status: deferred) — UIG condition 3 fails for each.
...
- **MC-cross-1** — Phase 02 capability "Transactional email sending service" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in `phase-02-auth/covers_capabilities` (backend slice — `phase-02-auth/TD-05` @nestjs-modules/mailer); (b) leave unclaimed if monolithic-fallback is acceptable.
- **MC-cross-2** — Phase 02 capability "User registration with email and password" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in `phase-02-auth-frontend/covers_capabilities` (and/or `phase-02-auth` if the backend signup endpoint is in scope); (b) leave unclaimed.
- **MC-cross-3** — Phase 02 capability "Automatic creation of the user's channel from the email prefix" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in `phase-02-auth/covers_capabilities` (backend — `phase-02-auth/TD-10` nickname allowlist); (b) leave unclaimed.
- **MC-cross-4** — Phase 02 capability "Account confirmation via email with an activation link" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in both slices (FE for landing, BE for token consumption); (b) leave unclaimed.
- **MC-cross-5** — Phase 02 capability "Login and user session control" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in both slices; (b) leave unclaimed.
...
- **MC-cross-7** — Phase 02 capability "Password recovery: request via email → link with token → reset" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in both slices; (b) leave unclaimed.
- **MC-cross-8** — Phase 02 capability "Registration, login, account confirmation, and password recovery screens" is not claimed by any slice's `covers_capabilities`. Explicit choice: (a) claim in `phase-02-auth-frontend/covers_capabilities` (FE-exclusive); (b) leave unclaimed.

## Resolved Issues

- **MD-1** _(resolved_by openapi-docs-nestjs/TD-02)_ — Decision #29 contract-sync strategy. `openapi-docs-nestjs/TD-02` ("OpenAPI Spec Artifact Strategy", Scope: Cross-layer) is in `## Inherited Decisions Detail` via correlator-confirm and satisfies the keyword-heuristic + Scope filter. Companion `next-frontend-openapi-typing/TD-01..TD-05` (FE consumption — openapi-typescript, openapi-fetch, committed sync, CI freshness check, `lib/api/contracts.ts`) close the chain.
...
- **OQ-8** _(resolved_by deferred_capability)_ — "Account confirmation via email with an activation link" marked deferred. Row in `## Non-UI / Deferred Capabilities`.
...
- **OQ-10** _(resolved_by deferred_capability)_ — "Password recovery (destination screen)" marked deferred. Row in `## Non-UI / Deferred Capabilities`.
...
- **UIG-1** _(resolved_by deferred_capability)_ — Capability "Registration, login, account confirmation, and password recovery screens" marked **deferred** in `## Non-UI / Deferred Capabilities`. Rationale (user-provided verbatim): "the account confirmation screen will not be implemented in the current phase, it will be postponed" — the umbrella bullet's full coverage requires the confirmation and reset-password destination screens, both already in Non-UI/Deferred rows above; the 3 ship-this-phase screens (signup, login, forgot-password) remain covered by their own inventory verbs. The umbrella bullet itself is deferred to the phase that lands the two missing screens.
