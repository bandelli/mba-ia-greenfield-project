...
sources_mtime:
  docs/project-plan.md: "2026-09-07T21:58:17"
  ...
  docs/phases/phase-01-configuracao-base/context.md: "2026-09-07T21:58:17"
  docs/phases/phase-02-auth/context.md: "2026-09-07T21:58:17"
  docs/phases/phase-02-auth-frontend/context.md: "2026-09-07T21:58:17"
...
|-----|--------|-------|-------|--------|----------|-----------|
| phase-03-videos/TD-01 | phase | Backend | Object Storage Backend & Client Library | decided | A (`@aws-sdk/client-s3`) | @aws-sdk/client-s3 |
...
| phase-03-videos/TD-07 | phase | Cross-layer | Media Delivery Mechanism for Streaming & Download | decided | A (Presigned/direct object-storage URLs) | @aws-sdk/s3-request-presigner |
| phase-03-videos/TD-08 | phase | Repo-wide | Docker Compose Topology for New Infrastructure Services | decided | A (Extend `nestjs-project/compose.yaml`) | — |
| phase-03-videos/TD-09 | phase | Backend | Upload Content Validation & Failure Cleanup Strategy | decided | C (Hybrid: onUploadCreate fast-reject + onUploadFinish authoritative check) | — |
| phase-03-videos/TD-10 | phase | Backend | Video Status Lifecycle & Processing Failure Handling | decided | A (Bounded automatic retry via pg-boss + persisted `error` terminal state) | — |
...
| Service with configured lib (e.g. `pg-boss`, `@aws-sdk/client-s3`) | Unit: real lib with test config |
| Service with side-effect dep (storage, queue) | Integration: real capture service (local MinIO/pg-boss) or local adapter |
...
| Middleware (e.g. the tus upload route mount) | E2E |

_No testing guide artifact exists yet for a background worker entrypoint (`src/worker/main.ts`, per TD-03) or for a pg-boss job-completion listener (TD-10) — closest existing coverage model is "Service with configured lib" + "Module with configured imports"; confirm during `/plan-build`._

### next-frontend

| Artifact created | Required tests |
|---|---|
| Route handler (`app/api/**/route.ts`) — proxy or with branching (e.g. stream-url / download-url issuers per TD-07) | `*.integration.test.ts` with MSW (+ `*.test.ts` for extracted pure logic) |
