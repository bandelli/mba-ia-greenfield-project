...
export type AppShellProps = {
  children: ReactNode
  // A pre-built slot (Suspense-wrapped in app/(main)/layout.tsx, around the
  // async Server Component that fetches the sidebar's subscribed-channels
  // section) rather than resolved data — this keeps that fetch from gating
  // AppShell's own render, which would otherwise delay `children` (the
  // routed page, with its own loading.tsx) behind it. `null`/anonymous and
  // the empty state are the responsibility of whatever fills the slot.
...
function AppShell({ children, subscriptionsSlot }: AppShellProps) {
