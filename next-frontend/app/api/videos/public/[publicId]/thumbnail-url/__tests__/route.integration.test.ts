...
import {
  ...
  VIDEO_THUMBNAIL_NOT_FOUND_TRIGGER,
...
beforeAll(async () => {
  ({ GET } = await import(
    "@/app/api/videos/public/[publicId]/thumbnail-url/route"
  ));
});

describe("GET /api/videos/public/[publicId]/thumbnail-url", () => {
  it("forwards to the upstream endpoint and returns the same format", async () => {
    const res = await GET(
      new Request("http://localhost/api/videos/public/pub123/thumbnail-url"),
  ...
  it("passes through a 404 VIDEO_NOT_FOUND without reshaping", async () => {
    const res = await GET(
      new Request(
        `http://localhost/api/videos/public/${PUBLIC_VIDEO_NOT_FOUND_TRIGGER}/thumbnail-url`
      ),
      {
  ...
  it("passes through a 404 VIDEO_THUMBNAIL_NOT_FOUND without reshaping", async () => {
    const res = await GET(
      new Request(
        `http://localhost/api/videos/public/${VIDEO_THUMBNAIL_NOT_FOUND_TRIGGER}/thumbnail-url`
      ),
      {
        params: Promise.resolve({
          publicId: VIDEO_THUMBNAIL_NOT_FOUND_TRIGGER,
        }),
      }
    );
