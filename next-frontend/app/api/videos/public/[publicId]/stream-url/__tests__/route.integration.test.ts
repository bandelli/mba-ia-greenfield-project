...
beforeAll(async () => {
  ({ GET } = await import(
    "@/app/api/videos/public/[publicId]/stream-url/route"
  ));
});

describe("GET /api/videos/public/[publicId]/stream-url", () => {
  it("forwards to the upstream endpoint and returns the same format", async () => {
    const res = await GET(
      new Request("http://localhost/api/videos/public/pub123/stream-url"),
  ...
  it("passes through a 404 VIDEO_NOT_FOUND without reshaping", async () => {
    const res = await GET(
      new Request(
        `http://localhost/api/videos/public/${PUBLIC_VIDEO_NOT_FOUND_TRIGGER}/stream-url`
      ),
