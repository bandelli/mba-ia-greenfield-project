...
beforeAll(async () => {
  ({ PUT } = await import("@/app/api/comments/[commentId]/reaction/route"));
});
...
function makeRequest(commentId: string, body: Record<string, unknown>) {
  return new Request(`http://localhost/api/comments/${commentId}/reaction`, {
    method: "PUT",
...
describe("PUT /api/comments/[commentId]/reaction", () => {
  it("forwards the request body and returns the upstream response unchanged", async () => {
    const res = await PUT(makeRequest("comment-1", { type: "dislike" }), {
      params: Promise.resolve({ commentId: "comment-1" }),
    });

    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body).toEqual({ type: "dislike", likesCount: 0, dislikesCount: 1 });
  });

  it("passes through a 404 COMMENT_NOT_FOUND without reshaping", async () => {
    const res = await PUT(
      makeRequest(REACTION_NOT_FOUND_TRIGGER, { type: "like" }),
      { params: Promise.resolve({ commentId: REACTION_NOT_FOUND_TRIGGER }) }
    );
  ...
  it("passes through a 401 UNAUTHORIZED without reshaping", async () => {
    server.use(
      http.put(`${env.API_URL}/comments/:commentId/reaction`, () =>
        HttpResponse.json(
    ...
    const res = await PUT(makeRequest("comment-1", { type: "like" }), {
      params: Promise.resolve({ commentId: "comment-1" }),
    });
  ...
  it("passes through a 400 validation error without reshaping", async () => {
    ...
    const res = await PUT(makeRequest("comment-1", { type: "invalid" }), {
      params: Promise.resolve({ commentId: "comment-1" }),
    });
