...
Video playback and download use presigned URLs the backend issues against Object Storage (S3/MinIO) — `GET /videos/public/{publicId}/stream-url` and `.../download-url` (proxied through `app/api/videos/public/[publicId]/stream-url/route.ts` etc.), consumed by `app/(main)/watch/[publicId]/page.tsx`.
